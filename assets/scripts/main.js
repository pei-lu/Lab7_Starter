...
function initializeServiceWorker() {
  ...
  if ("serviceWorker" in navigator) {
    ...
    console.log("Service worker fails");
  }
...
async function getRecipes() {
  ...
  if (recipes.length > 0) {
    // Recipes exist in local storage
    // Perform further actions or return the recipes as needed
    return recipes;
  } 
  /**************************/
  ...
  //            declared above
    ...
    for(const recipeURL of RECIPE_URLS) {
      try {
        ...
        if (numFectched == RECIPE_URLS.length){
